...
export function App() {
  ...
  // Safe items for the marquee
  const pulseItems = t.pulseItems || [];
    ...
    <div className="min-h-screen bg-[#f8faf9] selection:bg-[#2D7A4C] selection:text-white">
      ...
      <div className="bg-[#020617] overflow-hidden h-12 flex items-center border-b border-[#1e293b] sticky top-0 z-[110]">
        <div className="flex items-center animate-marquee whitespace-nowrap px-6">
          {pulseItems.concat(pulseItems).map((item: string, i: number) => (
            ...
            <motion.div key="explore" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="pt-12 md:pt-16">
              ...
              {state.cargando ? (
                <div className="py-40 flex flex-col items-center justify-center gap-10 text-center px-10">
                   ...
                   <p className="text-3xl font-serif italic text-[#475569] max-w-xl leading-relaxed">
                     "{t.indexingMsgs ? t.indexingMsgs[loadingMsgIdx] : 'Cargando...'}"
                   </p>
                </div>
              ) : (
      ...
      <AnimatePresence>
        {showPaymentModal && (
          <div className="fixed inset-0 z-[1000] flex items-center justify-center p-6">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.9, y: 40 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 40 }} className="relative w-full max-sm bg-[#13011a] rounded-[64px] overflow-hidden shadow-[0_50px_120px_rgba(0,0,0,0.8)] flex flex-col items-center text-white border border-white/5">
